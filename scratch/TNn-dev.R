...
OUT_DIR   <- file.path(INPUT_DIR, 'TNn'); dir.create(OUT_DIR)

# Load libraries and helper functions, this will also have to change.
source(here::here('scratch', '0.fxns.R'))


# Define the TNn function to calculate the monthly min 
# 
# Args
#   file: path to the tasmin file to process
#   OUT_DIR: the location where to write the output to 

calculate_TNn <- function(file, OUT_DIR){
  
  assert_that(grepl(pattern = 'tasmin', x = file))
  
  out_file <- file.path(OUT_DIR, paste0('TNn-', basename(file)))
  if(file.exists(out_file)) file.remove(out_file)
  
  system2('cdo', args = c('-a -monmin', file, out_file))
  return(out_file)
  
}


# 1. Calculate monthly minimum value of daily minimum temperature -------------------------
